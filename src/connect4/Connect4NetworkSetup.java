...
/**
 * This class serves as the Stage for the Network Setup dialog box.
 * 
 * <p>This class presents options for the user to select whether to create a game
 * as a Server or Client, play as a Human or Computer, as well as enter the Server
 * and Port number to connect to.</p>
 * 
...
public class Connect4NetworkSetup extends Stage{
    ...
    private RadioButton humanRB;
    private RadioButton computerRB;
    ...
    /**
     * <ul><b><i>getPlayAsToggleRow</i></b></ul>
     * <ul><ul><p><code>private HBox getPlayAsToggleRow () </code></p></ul>
     *
     * Sets up the 'Play as:' row of the Network Setup options.
     * 
     * <p>These elements are added to in an HBox for uniformity of display. The elements
     ...
     */    private HBox getPlayAsToggleRow() {
        ...
        humanRB = new RadioButton("Human");
        humanRB.setToggleGroup(playAsTG);
        humanRB.setSelected(true);
        computerRB = new RadioButton("Computer");
        computerRB.setToggleGroup(playAsTG);
        
        HBox row = new HBox(playAsLabel, humanRB, computerRB);
        row.setSpacing(PADDING);
        return row;
    }
    ...
    private void setScene() {
        ...
        HBox row1 = getCreateToggleRow();
        HBox row2 = getPlayAsToggleRow();
        
        // row3 (server/port row)
        Label serverLabel = new Label("Server");
        serverTF = new TextField(DEFAULT_SERVER);
        Label portLabel = new Label("Port");
        portTF = new TextField(DEFAULT_PORT);
        
        HBox row3 = new HBox(serverLabel, serverTF, portLabel, portTF);
        ...
        HBox row4 = new HBox(okay, cancel);
        row4.setSpacing(PADDING);
  
        
        // Adding rows to Vbox
        VBox box = new VBox(row1, row2, row3, row4);;
    ...
    /**
     * <ul><b><i>getPlayAsSelection</i></b></ul>
     * <ul><ul><p><code>public boolean getPlayAsSelection () </code></p></ul>
     *
